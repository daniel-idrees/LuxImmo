...
> Use when adding a module, wiring dependencies, checking the allowed dependency direction, or deciding how to structure a project. Which approach applies is decided once per project.
...
> **Offline-first toggle:** `:core:database` exists only when the app is offline-first. When it is **not**, omit the `:core:database` module entirely and drop it from `:core:data`'s dependencies — `:core:data` then depends only on `:core:domain` and `:core:network`. See [`layers.md`](layers.md) → *Network-only (no offline-first) variant*.
...
> **Offline-first toggle:** the `data/database/` package (Room) exists only when the app is offline-first. When it is **not**, omit it and use the network-only repository (`data` then uses only `domain` and `network`). See [`layers.md`](layers.md) → *Network-only (no offline-first) variant*.
